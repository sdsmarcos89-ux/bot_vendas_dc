...
class RestoreBackupModal(Modal):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs, title="🔄 Restaurar Backup")
        self.add_item(TextInput(label="Conteúdo do products.json", style=discord.InputTextStyle.long, required=True))
    async def callback(self, interaction: discord.Interaction):
        try:
            global PRODUCTS
            PRODUCTS = json.loads(self.children[0].value)
            save_products(PRODUCTS)
            await interaction.response.send_message("✅ Backup restaurado!", ephemeral=True)
        except: await interaction.response.send_message("❌ JSON inválido!", ephemeral=True)
...
class AdminProductEditView(View):
    ...
    async def edit_plans(self, b, i): await i.response.send_message(f"Gerenciar planos de **{PRODUCTS[self.product_id]['name']}**:", view=AdminProductEditPlanView(self.product_id), ephemeral=True)
...
class AdminProductStockPlanSelectView(View):
    def __init__(self, product_id, modal_class):
        super().__init__(timeout=None)
        opts = [discord.SelectOption(label=f"{pl['name']} ({get_available_stock_count(product_id, plid)})", value=plid) for plid, pl in PRODUCTS[product_id]["plans"].items()]
        if opts:
...
class AdminPanelMainView(View):
    ...
    @discord.ui.button(label="📦 Abastecer", style=discord.ButtonStyle.primary, row=1)
    ...
    async def del_p(self, b, i):
        ...
        sel = Select(options=opts)
        async def sel_cb(i2):
            del PRODUCTS[sel.values[0]]; save_products(PRODUCTS)
            await i2.response.send_message("✅ Excluído!", ephemeral=True)
        sel.callback = sel_cb
    ...
    async def back(self, b, i):
        ...
        with open(PRODUCTS_FILE, "rb") as f: await i.followup.send("📦 Backup:", file=discord.File(f, "products.json"), ephemeral=True)
    @discord.ui.button(label="🔄 Restaurar", style=discord.ButtonStyle.danger, row=2)
    async def rest(self, b, i): await i.response.send_modal(RestoreBackupModal())

@bot.event
async def on_ready(): print(f"Bot Online: {bot.user}")

@bot.slash_command(name="definirproduto", description="Vitrine de um produto")
async def definir(ctx, produto: discord.Option(str, "Nome do produto", autocomplete=lambda ctx: [p["name"] for p in PRODUCTS.values()])):
    ...
    emb.add_field(name="📦 Estoque Total", value=f"``` {get_total_product_stock_count(pid)} ```")
...
async def prods(ctx):
    ...
    await ctx.respond("Escolha um produto:", view=ProductSelectView())
...
@bot.slash_command(name="backup", description="Backup dos dados")
async def back_cmd(ctx):
    if ctx.author.id != OWNER_ID: return await ctx.respond("Apenas o dono!", ephemeral=True)
    with open(PRODUCTS_FILE, "rb") as f: await ctx.respond("📦 Backup:", file=discord.File(f, "products.json"), ephemeral=True)
